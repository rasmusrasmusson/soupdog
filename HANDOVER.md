...
DESIGN FORK to settle (doc first, [OPEN] flags, like every other spine piece):
1. **Representation:** extend `parent_id` tree (simple, one-parent, no overlap) vs build
   a proper m2m concept layer (concept + concept_members — matches the recipe-model
   concept-fork doc's "overlapping concepts, e-commerce pattern", handles an ingredient
   in two concepts). Reconcile with existing parent_id (doc suggests parent_id for
   product↔category, concepts for perceptual grouping).
2. **Nutrition resolution (the angle that surfaced this):** does the concept carry
   canonical "generic olive oil / generic apple" nutrition with variants OVERRIDING
   specific values, or is nutrition variant-only and the concept just a grouping? If
   concept-canonical, the sparse-vs-rich USDA mismatch stops mattering (recipe reads the
   concept's nutrition).
3. **Recipe reference:** does a recipe/plan point at a concept ("olive oil") or a
   specific variant ("EVOO")? What does the demand model use?
4. **Migration:** re-home existing variant-dupes (3 olive oils, apple varieties) WITHOUT
   breaking version_ingredients / recipes FKs (dry-run discipline like the dedup).
5. **Curation:** human decides what's a variant of what ("system measures & suggests,
   human names & decides").
...
- **Decompose model = Haiku 4.5 — ADOPTED.** Env-configurable:
  ...
  faster, ~3x cheaper. **[WATCH] Beef Wellington 502'd once (Anthropic call at 27.85s, NOT
  the 240s ceiling) then succeeded on retry** — one sample; if convergence-heavy recipes 502
  as a PATTERN, add auto-retry in the decompose route or route those to Sonnet (env-toggle
  supports a per-context split). Harness now reports `modelUnderTest` + per-case `decomposeMs`.
...
- **Wellington-502 watch** — add auto-retry in decompose route if convergence 502s recur.
