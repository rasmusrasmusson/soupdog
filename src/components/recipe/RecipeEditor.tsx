...
function GroupEditor({ group, groupIndex, totalGroups, ingredientTree, equipmentTree,
  ...
  // Always-current ref to avoid stale closures
  const groupRef = useRef(group);
  useEffect(() => { groupRef.current = group; }, [group]);
  // On mount: auto-calc yield if group has name but no saved value
  useEffect(() => {
    if (group.outputName.trim() && !group.outputQuantityValue) {
      const calc = calculateGroupYield(group.steps, ingredientTree);
      if (calc > 0) onChange({ ...group, outputQuantityValue: calc, outputQuantityUnit: group.outputQuantityUnit ?? 'g' });
    }
          ...
          {group.outputName.trim() && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 6, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', gap: 4, alignItems: 'center', flexWrap: 'wrap' }}>
                {/* Yield shown in all 4 units — value always stored in grams */}
                {(['g','kg','ml','l'] as const).map(u => {
                  const storedG = group.outputQuantityValue ?? 0;
                  const displayVal = (u === 'kg' || u === 'l')
                    ? Math.round(storedG / 1000 * 10000) / 10000
                    : storedG;
                  const isActive = (group.outputQuantityUnit ?? 'g') === u;
                  return (
                    <button key={u}
                      onClick={() => onChange({ ...groupRef.current, outputQuantityUnit: u })}
                      style={{
                    ...
                    >
                      <span style={{ color: isActive ? '#fff' : 'var(--fg)', fontWeight: 600 }}>
                        {storedG ? displayVal.toLocaleString('en-US') : '—'}
                      </span>
                      <span style={{ color: isActive ? 'rgba(255,255,255,0.8)' : 'var(--muted)', marginLeft: 2 }}>{u}</span>
                    </button>
                  );
                })}
                <input
                  type="text" inputMode="decimal"
                  value={group.outputQuantityValue ? Math.round(group.outputQuantityValue).toLocaleString('en-US') : ''}
                  placeholder="g"
                  onChange={e => {
                  ...
                  title="Override in grams"
                />
                {yieldUserEdited ? (
                  <button
                    onClick={() => {
                      const calc = calculateGroupYield(groupRef.current.steps, ingredientTree);
                      if (calc > 0) { onChange({ ...groupRef.current, outputQuantityValue: calc }); setYieldUserEdited(false); }
                    }}
                  ...
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--muted)' }}>auto</span>
                )}
              </div>
            </div>
          )}
