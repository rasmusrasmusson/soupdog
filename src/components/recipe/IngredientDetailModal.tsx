      ...
      <div onClick={e => e.stopPropagation()}
        ...
        {amount != null && nutrient && (
          <div style={{ fontFamily: MONO, fontSize: 12, color: 'var(--fg)', marginBottom: 14,
            ...
            <span style={{ fontSize: 15, color: 'var(--accent)' }}>{fmt(amount, nutrient.unit)} {nutrient.unit}</span>
...
function fmt(a: number, unit?: string): string {
  // Match the recipe table's rounding — precision should reflect the estimate's
  // confidence, not imply lab accuracy. kcal/mg/µg → whole; grams → whole, 1
  // decimal only below 1 g; unknown units → 1 decimal.
  const u = (unit || '').toLowerCase();
  if (u === 'kcal' || u === 'mg' || u === 'µg' || u === 'mcg' || u === 'iu') return Math.round(a).toLocaleString();
  if (u === 'g') return a < 1 ? a.toLocaleString(undefined, { maximumFractionDigits: 1 }) : Math.round(a).toLocaleString();
