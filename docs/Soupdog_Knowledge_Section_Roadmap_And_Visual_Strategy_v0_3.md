# Soupdog — Knowledge Section: Build Roadmap, Content Model & Visual Strategy — Design v0.3

**Status:** DESIGN (not built). 2026-06-07. v0.3 = v0.2 + §9 (a second content AREA: the
DIETARY-FIT knowledge — nutrients, allergies, religious/ethical preferences, diets — the
meal-planning reasoning layer surfaced as reading). v0.2 added the VALIDATED visual system
(§5). Covers the public-facing KNOWLEDGE SECTION — now spanning BOTH the cooking-knowledge
(Techniques, Tools, Ingredients) AND the dietary-fit knowledge (§9). Sits alongside (does
NOT supersede) the Culinary Knowledge Layer doc (v0.5, AI-guide + schema). This is the
READER-FACING + production side. Discard any v0.1/v0.2 copy.
...
## 9. Second content AREA: dietary-fit knowledge (meal-planning reasoning, as reading)

The cooking-knowledge sections (Techniques/Tools/Ingredients) are the HOW-TO-COOK spine.
A second, parallel area is the WHY-THIS-SUITS-THIS-PERSON spine — the reasoning behind meal
planning, surfaced as reader-facing reference. Same "one spine, two faces" principle: data
the planner ALREADY uses (or will), turned into an encyclopedia. These cross-link richly into
the cooking content (an allergen → ingredients containing it; Halal → governed ingredients/
methods; a nutrient → ingredients rich in it). Likely its own nav grouping (e.g. "Nutrition &
Diet" / "Eating") alongside the cooking sections. NONE built; this is design capture, and it's
DOWNSTREAM of the Tools/curation work already queued.

### The four content types (sequenced by readiness + value, NOT all equal)

1. **Nutrients — BUILD FIRST of these.** A page per nutrient (protein, fibre, sodium, vitamin
   C, …): what it does, how much you need, what it's in, too much/too little. Highest reader
   value, cleanest fit (we already hold `nutrition_per_100g`; the demand model uses nutrient
   targets), lowest sensitivity. The BEST home for the data-driven graphs (§5): "ingredients
   richest in this", "% of daily target in a typical serving" — generated from our data. The
   flagship of this area.

2. **Allergies — high value, build with GUARDRAILS.** A page per allergen (the EU-14 we already
   hold + others): what it is, where it's found, cross-contamination, what to watch for. Powerful
   cross-linking (allergen → every ingredient containing it, from our `allergens` data). BUT this
   is health-SAFETY content where being wrong can hurt someone → INFORMATIONAL, NOT ADVISORY:
   explain, don't give medical guidance/dosing/"safe-for-you" assurances; clear "not medical
   advice, consult a professional" framing.

3. **Religious / ethical preferences (Halal, Kosher, …) — valuable, HIGHEST curation bar.**
   "What is Halal" (meaning, origins, what it governs in food, effect on ingredients/prep) is
   good reading and ties to existing `is_halal`/`is_kosher` flags. CAUTION: religious/cultural
   practice is contested and varies between traditions/authorities; easy to flatten or get
   subtly wrong/offensive. Must be DESCRIPTIVE and RESPECTFUL, ATTRIBUTING VARIATION ("practices
   vary; some authorities hold X, others Y"), and HUMAN-REVIEWED before going live (AI drafts a
   careful sourced first pass; this is the category where human review matters most).

4. **Diets — descriptive only, LOWER priority, "(maybe)".** Keto/paleo/Mediterranean/vegan/etc.
   The muddiest category: loosely defined, faddish, often contested or pseudo-scientific; some
   (weight-loss) brush wellbeing concerns. A neutral "what is X / its principles" page is fine
   and useful; do NOT rank/recommend diets or frame restrictive dieting aspirationally; avoid
   health claims and anything reading as weight-loss promotion. Scope tightly to explain-don't-
   prescribe. Lowest priority of the four.

### Cross-cutting principles for this area (DIFFERENT from cooking content)
- **Health-safety responsibility:** nutrients, allergies, diets touch health. Standing rule —
  INFORMATIONAL/EDUCATIONAL, never medical advice or prescriptive; clear framing; "consult a
  professional" where someone might ACT on it (especially allergy/diet). This is stricter than
  the cooking-content tone.
- **These pages double as the meal-planning EXPLAINER:** the natural place to explain WHY the
  planner avoided an allergen or honored a preference — closing the loop between the reasoning
  and the reader (as a technique page explains the AI's decomposition).
- **Respect + attribution for contested topics** (religion, diet): describe variation, don't
  assert one truth; human-review the sensitive ones.

### Sequencing
Nutrients → Allergies → Religious/ethical → Diets. All downstream of the cooking-knowledge
build (Tools section, curation, reader view-mode) — this area reuses the same page architecture
(reference entry + explanation + cross-links + reader/data faces), so it's additive, not new
infrastructure. Build when the cooking sections are solid and there's reason to widen.
