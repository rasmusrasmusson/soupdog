...
**Document:** Soupdog_Nutrition_Data_Sourcing_Design_v0_3
**Status:** Design — §10 SETTLED; Phase 1 BUILT & LIVE; USDA data shape VALIDATED (see §12).
**Date:** 2026-06-22 (v0.3 — Phase 1 shipped + real USDA findings folded in)
**Supersedes:** v0_1, v0_2 (delete them)
...
## 12. BUILD STATE — Phase 1 shipped & live (2026-06-22)

Phase 1 (schema) is BUILT, on production, verified. What exists now:
- `nutrient` lookup — 16 rows seeded (the 15 live blob keys + calcium), each with
  `fdc_nutrient_id`, `category`, `unit`, `display_order`. **All 16 fdc_nutrient_id values were
  verified against a live USDA response (olive oil) — every id matches.** See §13.
- `ingredient_nutrient_value` — per-(ingredient, nutrient, source_kind) rows; unique index on
  that triple; public-scoped RLS + grants. 3,146 rows migrated from the old blob, all
  `e0_inferred / ai`, across 308 ingredients.
- `ingredient_nutrition_current` — resolved view (DISTINCT ON (ingredient,nutrient),
  ordered evidence_rank DESC → source-preference → measured_at DESC). Mirrors
  `target_state_rules_current`.
- **Read path repointed:** `/api/recipes/[id]/nutrition/route.ts` now sources per-100g values
  from the view (not the blob), assembling the same `nutritionPer100g` shape the calculator
  expects. Verified: Greek salad returns identical numbers post-repoint (coveredPct 100,
  calories 698, sodium 1707.5, totalWeightG 1279) — proving the view-sourced path matches what
  the blob held.
- **Blob NOT dropped.** `ingredients.nutrition_per_100g` remains as a fallback-only relic (the
  recipe page's client-side fallback still reads it). Drop it AFTER Phase 2, once the page
  fallback is also repointed and the view definitively holds richer data.

SQL files used (run in Supabase editor, not repo migrations): `nutrition_phase1_dryrun.sql`,
`nutrition_phase1_build.sql`.

## 13. USDA data shape — VALIDATED against live API (probe: olive oil)

A live `foods/search` call (data.gov key; Foundation + SR Legacy) confirmed the real payload and
reshaped three design points. **All findings are from FDC 167737 ("Oil, corn, peanut, and
olive", SR Legacy) — see the curation caveat below.**

### 13a. fdc_nutrient_id mappings — CONFIRMED correct
Energy 1008 (KCAL), protein 1003, total fat 1004, carbs 1005, saturated 1258, fiber 1079,
sugar 2000, sodium 1093, potassium 1092, iron 1089, magnesium 1090, phosphorus 1091, calcium
1087, vitamin C 1162, vitamin B6 1175, folate 1177 — every Phase-1 seed id matches live data.

### 13b. Energy needs UNIT disambiguation [import rule]
USDA returns BOTH id 1008 "Energy" in KCAL (884) AND id 1062 "Energy" in kJ (3700). The import
must select **id 1008 AND unitName = 'KCAL'**, never just the first "Energy" row. (General rule:
match on nutrient id *and* expected unit; a few nutrients appear in multiple units / forms.)

### 13c. Omega-6 / omega-3 are DERIVED, not stored as single fields [modeling decision]
There is NO `omega_6` / `omega_3` USDA nutrient. They are sums of fatty-acid isomers:
- **omega-6** = n-6 PUFAs — primarily PUFA 18:2 (id 1269, linoleic), plus 20:4 (1271, arachidonic).
- **omega-3** = n-3 PUFAs — PUFA 18:3 (1270, ALA), plus EPA 20:5 (1278), DHA 22:6 (1272), DPA
  22:5 (1280).
DECISION: **store the individual fatty-acid isomers** (ground truth; food-industry customers want
the by-isomer breakdown, not just a rollup) and **compute omega-6/omega-3 rollups at DISPLAY**
(or as a derived nutrient row flagged `source_kind='derived'`). Do NOT invent stored omega fields
that lose the underlying detail.

### 13d. Nutrient table must EXPAND in Phase 2 [build task]
One food carried ~80 nutrients. Phase 2 adds (at least) the valuable ones beyond the seeded 16:
- **Fatty-acid isomers** (category `fatty_acid`): 1259–1280 individual SFA/MUFA/PUFA chains,
  plus totals MUFA 1292 / PUFA 1293. (Saturated total 1258 already seeded.)
- **Vitamins**: E α-tocopherol 1109, K phylloquinone 1185, A RAE 1106, D 1114, thiamin 1165,
  riboflavin 1166, niacin 1167, B12 1178, pantothenic 1170, choline 1180.
- **Minerals**: zinc 1095, copper 1098, manganese 1101, selenium 1103.
- **Other**: water 1051, ash 1007 (probably skip for display).
Each gets `category`, `unit`, and a `display_order` (null = imports but not shown by default —
amino acids / rare isomers exist in the table but only surface when curated into the UI).

### 13e. Matching MUST be curated — rank-1 is wrong [confirms §6]
The top "olive oil" hit was **a corn/peanut/olive BLEND**, not pure olive oil — different
fatty-acid profile. Proof that naive top-result matching gives *an* answer, not the *right* one.
Phase 2 matching must surface the FDC `description` + `dataType` so a human sees "that's a blend"
and picks the correct pure-ingredient food. AI suggests candidates with descriptions; human
confirms. (Foundation Foods preferred over SR Legacy at equal relevance, per §10.2.)

### 13f. API mechanics [build notes]
`GET https://api.nal.usda.gov/fdc/v1/foods/search?query=…&dataType=Foundation,SR Legacy&pageSize=…&api_key=…`
for matching; `GET /fdc/v1/food/{fdcId}` for the full profile on a confirmed match. Free data.gov
key, ~1,000 req/hr. Bulk JSON/CSV download (no limit) for the initial mass ingest. **Key lives in
Vercel env (`USDA_FDC_API_KEY`), never in code/chat.**
